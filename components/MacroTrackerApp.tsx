...
export default function MacroTrackerApp() {
  ...
  const waterGoal = activeProfile?.customWaterMl && activeProfile.customWaterMl > 0 ? activeProfile.customWaterMl : (activeProfile ? Math.max(1500, Math.round(activeProfile.weightKg * 35)) : 2000);
  const lang: Lang = state.lang || "fr";
  const tr = makeT(lang);
  const weightGoal = (() => {
    const tw = activeProfile?.targetWeightKg;
    if (!tw || tw <= 0) return null;
    const cur = average7(state.weights);
    if (cur == null) return null;
    const diff = Math.round((tw - cur) * 10) / 10; // + = prendre, - = perdre
    const reached = Math.abs(diff) < 0.3;
    const daysLeft = activeProfile?.targetDate ? Math.round((new Date(activeProfile.targetDate + "T12:00:00").getTime() - Date.now()) / 86400000) : null;
    const requiredPerWeek = (daysLeft != null && daysLeft > 0) ? Math.round((diff / (daysLeft / 7)) * 100) / 100 : null;
    const safeMax = diff < 0 ? Math.min(1, Math.round(cur * 0.01 * 100) / 100) : 0.5;
    const paceOk = requiredPerWeek == null ? null : Math.abs(requiredPerWeek) <= safeMax + 0.02;
    const trend = weightTrendPerWeek(state.weights);
    let etaDate: string | null = null;
    let movingToward: boolean | null = null;
    if (trend != null && !reached) {
      if (Math.abs(trend) < 0.05) movingToward = null; // poids stable
      else {
        movingToward = (diff < 0 && trend < 0) || (diff > 0 && trend > 0);
        if (movingToward) etaDate = new Date(Date.now() + (diff / trend) * 7 * 86400000).toLocaleDateString(lang === "es" ? "es-ES" : "fr-FR");
      }
    }
    return { cur, target: tw, diff, reached, daysLeft, requiredPerWeek, safeMax, paceOk, trend, etaDate, movingToward };
  })();
  ...
  function saveProfile(form: FormData) {
    ...
    const profile: Profile = {
      ...
      bodyFatPct: frNum(form.get("bodyFatPct"), 0) || undefined, activity: Number(form.get("activity") || 1.55), goal: form.get("goal") as Profile["goal"], proteinPerKg: frNum(form.get("proteinPerKg"), 0) || undefined, customBmr: frNum(form.get("customBmr"), 0) || undefined, customKcal: frNum(form.get("customKcal"), 0) || undefined, customWaterMl: frNum(form.get("customWaterMl"), 0) || undefined, targetWeightKg: frNum(form.get("targetWeightKg"), 0) || undefined, targetDate: String(form.get("targetDate") || "") || undefined, diet: form.get("diet") as DietType,
  ...
  function applyGoalCalories(kcal: number) {
    ...
    <header className="header hero">
      <div className="brand">
        ...
        <div className="hero-badges">
          ...
          <button type="button" className="hero-link" onClick={()=>setTab("journal")}>Quantités ajustables</button>
        </div>
      </div>
      <div className="profile-card clickable-nav" onClick={()=>setTab("profil")} role="button" tabIndex={0} title="Ouvrir le profil">
...
function TdeeSparkline({ points }: { points: { label: string; tdee: number }[] }) {
  if (points.length < 2) return null;
  const vals = points.map(p => p.tdee);
  const min = Math.min(...vals); const max = Math.max(...vals); const range = Math.max(1, max - min);
  const pts = points.map((p, i) => `${(i / (points.length - 1)) * 100},${100 - ((p.tdee - min) / range) * 80 - 10}`).join(" ");
  return <svg className="chart" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ height: 60 }}><polyline points={pts} fill="none" stroke="var(--primary)" strokeWidth="2" /></svg>;
}

function WeightChart({ weights, targetWeightKg, targetDate, trendPerWeek }: { weights: WeightLog[]; targetWeightKg?: number; targetDate?: string; trendPerWeek?: number | null }) {
  ...
  const t0 = t(data[0].date); const lastT = t(last.date);
  // Point de projection : soit vers l'objectif (poids + date), soit extrapolation de la tendance sur 4 semaines.
  let projEnd: { time: number; w: number } | null = null;
  if (targetWeightKg && targetWeightKg > 0 && targetDate && t(targetDate) > lastT) {
    projEnd = { time: t(targetDate), w: targetWeightKg };
  } else if (trendPerWeek != null && Math.abs(trendPerWeek) > 0.02) {
    projEnd = { time: lastT + 28 * DAY, w: Math.round((last.weightKg + trendPerWeek * 4) * 10) / 10 };
  }
  const tEnd = projEnd ? Math.max(lastT, projEnd.time) : lastT;
  const spanT = Math.max(1, tEnd - t0);
  const allW = data.map(d => d.weightKg);
  if (projEnd) allW.push(projEnd.w);
  if (targetWeightKg && targetWeightKg > 0) allW.push(targetWeightKg);
  const min = Math.min(...allW); const max = Math.max(...allW); const range = Math.max(1, max - min);
  const x = (time: number) => ((time - t0) / spanT) * 100;
  const y = (w: number) => 100 - ((w - min) / range) * 90 - 5;
  const pts = data.map(d => `${x(t(d.date))},${y(d.weightKg)}`).join(" ");
  const projPts = projEnd ? `${x(lastT)},${y(last.weightKg)} ${x(projEnd.time)},${y(projEnd.w)}` : null;
  const targetY = (targetWeightKg && targetWeightKg > 0) ? y(targetWeightKg) : null;
  return (
    <svg className="chart" viewBox="0 0 100 100" preserveAspectRatio="none">
      {targetY != null && <line x1="0" y1={targetY} x2="100" y2={targetY} stroke="currentColor" strokeWidth="0.6" strokeDasharray="1 2" opacity="0.35" />}
      {projPts && <polyline points={projPts} fill="none" stroke="currentColor" strokeWidth="1.4" strokeDasharray="3 2" opacity="0.55" />}
